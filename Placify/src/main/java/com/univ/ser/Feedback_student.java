...
/**
 * Servlet implementation class Feedback_student
 */
@WebServlet("/Feedback_student")
public class Feedback_student extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Feedback_student() {
        super();
		...
		//doGet(request, response);
		 String feedback = request.getParameter("feedback");
	        ...
	        if (session == null || session.getAttribute("enrollmentNo") == null) {
                ...
                request.getRequestDispatcher("student_academic_feedback.jsp").forward(request, response);
	        ...
	        try(Connection conn = DBConnection.getConn()) {
	            String sql = "INSERT INTO feedback_student (id,name, year, feedback) VALUES (?, ?, ?, ?)";
